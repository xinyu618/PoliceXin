...
package com.pw.policeweixin.websocket;
...
import com.pw.policeweixin.config.BuildConfig;


/**
 * WebSocket writer, the sending leg of a WebSockets connection. This is run on it's background thread with it's own
 * message loop. The only method that needs to be called (from foreground thread) is forward(), which is used to forward
 * a WebSockets message to this object (running on background thread) so that it can be formatted and sent out on the
...
public class WebSocketWriter extends Handler {
	 ...
	 *
	 * @param looper
	 *            The message looper of the background thread on which this object is running.
	 * @param master
	 ...
	 */
	public WebSocketWriter(final Looper looper, final Handler master, final SocketChannel socket,
	...
	/**
	 * Call this from the foreground (UI) thread to make the writer (running on background thread) send a WebSocket
	 * message on the underlying TCP.
	 *
	 * @param message
	 *            Message to send to WebSockets writer. An instance of the message classes inside WebSocketMessage or
	 *            another type which then needs to be handled within processAppMessage() (in a class derived from this
	 *            class).
	 */
	public void forward(final Object message) {
	 ...
	 */
	private void notify(final Object message) {
	 ...
	 */
	private void sendClientHandshake(final WebSocketMessage.ClientHandshake message) throws IOException {
	 ...
	 */
	private void sendClose(final WebSocketMessage.Close message) throws IOException, WebSocketException {
	 ...
	 */
	private void sendPing(final WebSocketMessage.Ping message) throws IOException, WebSocketException {
	...
	/**
	 * Send WebSockets pong. Normally, unsolicited Pongs are not used, but Pongs are only send in response to a Ping
	 * from the peer.
	 */
	private void sendPong(final WebSocketMessage.Pong message) throws IOException, WebSocketException {
		if (message.mPayload != null && message.mPayload.length > 125) {
	 ...
	 */
	private void sendBinaryMessage(final WebSocketMessage.BinaryMessage message) throws IOException, WebSocketException {
	 ...
	 */
	private void sendTextMessage(final WebSocketMessage.TextMessage message) throws IOException, WebSocketException {
	 ...
	 * Send WebSockets binary message.
	 */
	private void sendRawTextMessage(final WebSocketMessage.RawTextMessage message) throws IOException,
			WebSocketException {
		if (message.mPayload.length > mOptions.getMaxMessagePayloadSize()) {
			throw new WebSocketException("message payload exceeds payload limit");
		}
	...
	/**
	 * Sends a WebSockets frame. Only need to use this method in derived classes which implement more message types in
	 * processAppMessage(). You need to know what you are doing!
	 *
	 * @param opcode
	 ...
	 */
	protected void sendFrame(final int opcode, final boolean fin, final byte[] payload) throws IOException {
		if (payload != null) {
	 ...
	 */
	protected void sendFrame(final int opcode, final boolean fin, final byte[] payload, final int offset,
		...
		if (len <= 125) {
			...
			b1 |= (byte) (126 & 0xff);
			mBuffer.write(b1);
			mBuffer.write(new byte[] { (byte) ((len >> 8) & 0xff), (byte) (len & 0xff) });
		} else {
			b1 |= (byte) (127 & 0xff);
			mBuffer.write(b1);
			mBuffer.write(new byte[] { (byte) ((len >> 56) & 0xff), (byte) ((len >> 48) & 0xff),
	...
	/**
	 * Process message received from foreground thread. This is called from the message looper set up for the background
	 * thread running this writer.
	 *
	 * @param msg
	...
	/**
	 * Process WebSockets or control message from master. Normally, there should be no reason to override this. If you
	 * do, you need to know what you are doing.
	 *
	 * @param msg
	 *            An instance of the message types within WebSocketMessage or a message that is handled in
	 *            processAppMessage().
	 */
	...
	/**
	 * Process message other than plain WebSockets or control message. This is intended to be overridden in derived
	 * classes.
	 *
	 * @param msg
