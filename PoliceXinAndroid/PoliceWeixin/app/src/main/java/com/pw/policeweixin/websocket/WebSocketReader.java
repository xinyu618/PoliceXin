...
/**
 * WebSocket reader, the receiving leg of a WebSockets connection. This runs on it's own background thread and posts
 * messages to master thread's message queue for there to be consumed by the application. The only method that needs to
 * be called (from foreground thread) is quit(), which gracefully shuts down the background receiver thread.
 */
public class WebSocketReader extends Thread {
	 ...
	 */
	public WebSocketReader(final Handler master, final SocketChannel socket, final WebSocketOptions options,
				...
				if (mFrameBuffer.position() >= header_len) {
					...
					if (payload_len1 == 126) {
						...
						payload_len = ((long) (0xff & mFrameBuffer.get(i + 0)) << 56)
								...
								| ((long) (0xff & mFrameBuffer.get(i + 6)) << 8) | (0xff & mFrameBuffer.get(i + 7));
				...
				if (mFrameHeader.mOpcode > 7) {
					...
					if (mFrameHeader.mOpcode == 8) {
						...
						if (mFrameHeader.mPayloadLen >= 2) {
							...
							if (code < 1000
									|| (code >= 1000 && code <= 2999 && code != 1000 && code != 1001 && code != 1002
											&& code != 1003 && code != 1007 && code != 1008 && code != 1009
											&& code != 1010 && code != 1011) || code >= 5000) {
	...
	/**
	 * WebSockets text message received, default notifies master. This will only be called when the option
	 * receiveTextMessagesRaw HAS NOT been set.
	 * 
	...
	/**
	 * WebSockets text message received, default notifies master. This will only be called when the option
	 * receiveTextMessagesRaw HAS been set.
	 * 
	...
	private boolean processHandshake() throws UnsupportedEncodingException {
		...
		for (int pos = mFrameBuffer.position() - 4; pos >= 0; --pos) {
			if (mFrameBuffer.get(pos + 0) == 0x0d && mFrameBuffer.get(pos + 1) == 0x0a
					&& mFrameBuffer.get(pos + 2) == 0x0d && mFrameBuffer.get(pos + 3) == 0x0a) {
				...
				if (mFrameBuffer.get(0) == 'H' && mFrameBuffer.get(1) == 'T' && mFrameBuffer.get(2) == 'T'
	...
	@SuppressWarnings("unused")
	private Map<String, String> parseHttpHeaders(final byte[] buffer) throws UnsupportedEncodingException {
	...
	public void run() {
		...
		try {
			...
			do {
				...
				//Log.v(TAG,"len:"+len+" mFrameBuffer:"+mFrameBuffer);
				//sleep(500);
			} while (!mStopped);
